...
> **Gotta learn 'em all.**
> A daily knowledge "Pokédex" — every card is visible, but a card only joins the profile after the reader solves a logic/math challenge.

FielDex is a learning-in-public project: a personal commitment to learn something new every single day, turn it into a polished collectible card, and keep a public, growing dex of it all. The full gallery is open for reading; the collection profile is earned through small reasoning puzzles. The name is a play on **Pokédex** — but instead of catching creatures, I'm catching knowledge — with a small nod to *o Timão* (the club's nickname, "a Fiel", founded in **1910**).
...
Each entry is typed (like a Pokémon type) and has a rarity. The web gallery shows every card by default, but local profile progress only advances after the reader answers that card's deterministic logic/math puzzle.
